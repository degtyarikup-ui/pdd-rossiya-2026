...
const BRAND_SVGS = {
  ...
  appstore: '<svg viewBox="0 0 24 24" width="14" height="14" fill="#38bdf8" style="flex-shrink:0;"><path d="M18.7 19.5c-.8 1.2-1.7 2.4-3 2.4-1.4 0-1.8-.8-3.4-.8-1.6 0-2.1.8-3.4.8-1.3 0-2.3-1.3-3.1-2.5C4.2 17 3 13.6 3 10.4c0-5.1 3.3-7.8 6.5-7.8 1.7 0 3.3 1.2 4.3 1.2 1 0 2.9-1.5 4.9-1.3.8 0 3.2.3 4.7 2.5-3.9 2.3-3.3 7.5.7 9.1-.8 2-1.9 4-3.4 5.4zM15.9 2.6c.8-1 1.3-2.3 1.2-3.6-1.1.1-2.5.7-3.3 1.7-.7.8-1.4 2.2-1.2 3.5 1.3.1 2.5-.6 3.3-1.6z"/></svg>',
  web: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#8b5cf6" stroke-width="2" style="flex-shrink:0;"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
  ...
  direct: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#64748b" stroke-width="2" style="flex-shrink:0;"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
  other: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#475569" stroke-width="2" style="flex-shrink:0;"><circle cx="12" cy="12" r="10"/><path d="M12 8v8M8 12h8"/></svg>'
};

// Brand configuration maps
const STORE_CONFIG = {
  appstore: { name: 'App Store', color: '#38bdf8', icon: BRAND_SVGS.appstore },
...
const SOURCE_CONFIG = {
  ...
  yandex: { name: 'Ян��екс', color: '#f59e0b', icon: BRAND_SVGS.yandex },
  ...
  organic_appstore: { name: 'App Store', color: '#38bdf8', icon: BRAND_SVGS.appstore },
  other: { name: 'Прочее', color: '#475569', icon: BRAND_SVGS.other }
};
...
function formatCountryBadge(code) {
  ...
  return '<span class="tag-badge" style="display:inline-flex;align-items:center;gap:5px;font-size:12px;font-weight:600;"><span style="font-size:14px;line-height:1;">' + flag + '</span> ' + c + '</span>';
}
...
function formatBrandBadge(srcRaw) {
  ...
  return '<span style="display:inline-flex;align-items:center;gap:7px;font-weight:600;white-space:nowrap;">'
    + conf.icon
    + '<span>' + conf.name + '</span>'
    + '</span>';
}
  ...
  if (!sourceGroupList.length) {
    ...
    tbodySources.innerHTML = sourceGroupList.map(s => {
      return '<tr>'
        ...
        + '<td><span style="color:#10b981;font-weight:600;">' + s.installs + '</span></td>'
        + '</tr>';
    }).join('');
  }
...
function getChartOptions(showLegend = true) {
  return {
    ...
    plugins: {
      legend: {
        display: showLegend,
        labels: { color: '#64748b', boxWidth: 10, usePointStyle: true, font: { size: 11.5 } }
      },
      tooltip: {
...
async function loadAdminReels() {
  ...
  try {
    ...
    container.innerHTML = cachedReels.map((r, idx) => {
      const countryBadge = formatCountryBadge(r.country || 'ru');
