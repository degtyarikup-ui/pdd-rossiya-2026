...
async function loadAdminReels() {
  ...
  try {
    ...
    container.innerHTML = cachedReels.map((r, idx) => {
      ...
      return '<div style="background:#ffffff;border:1px solid var(--card-border);border-radius:10px;padding:14px 16px;display:flex;gap:14px;align-items:center;flex-wrap:wrap;box-shadow:0 1px 2px rgba(0,0,0,0.03);">'
        + '<div onclick="openReelModal(\'' + safeUrl + '\', \'' + safeTitle + '\')" style="width:70px;height:95px;background:#f1f5f9;border:1px solid var(--card-border);border-radius:8px;cursor:pointer;flex-shrink:0;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:all 0.15s;" onmouseover="this.style.borderColor=\'var(--primary)\'" onmouseout="this.style.borderColor=\'var(--card-border)\'">'
        + '<div style="width:28px;height:28px;border-radius:50%;background:#e0f2fe;display:flex;align-items:center;justify-content:center;color:var(--primary);">'
        ...
        + '<button data-id="' + r.id + '" onclick="deleteReel(this.dataset.id)" class="btn-action" style="padding:4px 8px;font-size:11.5px;color:var(--danger);border-color:#fecaca;">Удалить</button>'
        + '</div>'
        + '</div>'
        + '<div style="font-size:14.5px;font-weight:700;color:var(--text);margin-bottom:4px;">' + displayTitle + '</div>'
        ...
        + '<button onclick="openReelModal(\'' + safeUrl + '\', \'' + safeTitle + '\')" style="background:var(--primary);color:#ffffff;border:none;border-radius:6px;padding:4px 10px;font-weight:600;font-size:11.5px;cursor:pointer;display:inline-flex;align-items:center;gap:5px;"><svg viewBox="0 0 24 24" width="11" height="11" fill="currentColor"><polygon points="6 4 20 12 6 20 6 4"/></svg> Смотреть</button>'
