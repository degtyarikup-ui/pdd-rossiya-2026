...
function renderDashboard(data) {
  ...
  // 2. Chart: App Installs Growth (Line)
  ...
  gradInstalls.addColorStop(0, 'rgba(16, 185, 129, 0.25)');
  ...
  // 3. Chart: Platforms & Stores (Doughnut)
  const ctxTargets = document.getElementById('chart-targets').getContext('2d');
  const targetLabels = (data.targets || []).map(t => t.name || 'Store');
  const targetClicks = (data.targets || []).map(t => t.clicks || 0);
  
  if (chartTargets) chartTargets.destroy();
  chartTargets = new Chart(ctxTargets, {
    type: 'doughnut',
    data: {
      labels: targetLabels.length ? targetLabels : ['Нет данных'],
      datasets: [{
        data: targetClicks.length ? targetClicks : [1],
        backgroundColor: ['#38bdf8', '#10b981', '#6366f1', '#f59e0b', '#64748b'],
        borderWidth: 0
      }]
    },
  ...
  // 5. Chart: Traffic Sources (Doughnut)
  const ctxSources = document.getElementById('chart-sources').getContext('2d');
  const srcLabels = (data.sources || []).map(s => s.name || 'Direct');
  const srcValues = (data.sources || []).map(s => s.views || s.clicks || 0);

  if (chartSources) chartSources.destroy();
  chartSources = new Chart(ctxSources, {
    type: 'doughnut',
    data: {
      labels: srcLabels.length ? srcLabels : ['Прямой переход'],
      datasets: [{
        data: srcValues.length ? srcValues : [1],
        backgroundColor: ['#ef4444', '#06b6d4', '#ec4899', '#38bdf8', '#0077FF', '#64748b'],
        borderWidth: 0
      }]
    },
    options: getDoughnutOptions()
