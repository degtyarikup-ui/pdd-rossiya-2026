...
// Ежечасный крон обслуживает автопостинг (см. wrangler.toml).
const HOURLY_CRON = '5 * * * *';
...
function renderAdminPage() {
  ...
  const html = ADMIN_HTML_HEAD
    .replace('</nav>', SOCIAL_NAV_HTML + '    </nav>')
    .replace('\n\n  </main>', '\n' + SOCIAL_VIEW_HTML + '\n  </main>');
  return html + "<script>" + ADMIN_CLIENT_JS + SOCIAL_CLIENT_JS + "</script></body></html>";
}
...
Официальный комментарий: ${officialExplanation || 'Нет официального комментария'}`;
  ...
  async scheduled(event, env, ctx) {
    ...
    // Ежечасный триггер обслуживает автопостинг: у каждого аккаунта своё время
    // публикации, поэтому расписание проверяется каждый час, а не дважды в день.
