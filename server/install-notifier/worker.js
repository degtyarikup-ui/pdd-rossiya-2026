...
async function getAdsConfig(env) {
  if (!env.INSTALLS) return DEFAULT_ADS_CONFIG;
  try {
...
function filterAdsForClient(masterConfig, platform, store) {
  if (!masterConfig.enabled) {
    return { enabled: false, frequency: 15, mode: 'none', cards: [] };
  }
  
  const normStore = store === 'gplay' || store === 'google' ? 'googleplay' : (store === 'apple' || store === 'ios' ? 'appstore' : store);
  const rule = masterConfig.storeRules?.[normStore] || masterConfig.storeRules?.['appstore'] || { enabled: true, mode: 'custom_cards' };
  
  if (!rule.enabled) {
    return { enabled: false, frequency: masterConfig.frequency || 15, mode: 'none', cards: [] };
  }
  
  const activeCards = (masterConfig.promoCards || []).filter(c => {
    if (!c.enabled) return false;
    if (!c.targetStores || c.targetStores.length === 0) return true;
    if (normStore && normStore !== 'all') {
      return c.targetStores.includes(normStore);
    }
    return true;
  });
  ...
  return {
    enabled: true,
    frequency: masterConfig.frequency || 15,
    mode: rule.mode || 'custom_cards',
    yandexAdUnitId: rule.yandexAdUnitId || '',
    cards: activeCards
  };
}
