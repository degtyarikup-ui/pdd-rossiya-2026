...
// Применяет событие к данным 12-часового слота (для отчётов в Telegram).
// Чистая функция: ничего не читает и не пишет в KV — запись делает
// flushBufferedStats одним махом для всей пачки событий.
...
function detectAppCode(data = {}) {
  const country = String(data.country || '').toLowerCase();
  const packageName = String(data.package || '').toLowerCase();
  ...
  if (country === 'rs' || packageName === 'rs.pdd.pdd_app' || appName === 'rs' ||
      appName.includes('srb') || appName.includes('serb') || path.includes('rs.') || target.startsWith('rs-')) {
    return 'rs';
  }
  if (country === 'by' || packageName === 'by.pdd.pdd_app' || appName === 'by' ||
      appName.includes('беларус') || appName.includes('belarus') || target.startsWith('by-')) {
    return 'by';
  }
  return 'ru';
}
...
async function getBlogArticles(env) {
  ...
  // Админские операции меняют элементы массива. Возвращаем копии, чтобы
  // состояние по умолчанию не мутировало между запросами одного isolate.
  return DEFAULT_BLOG_ARTICLES.map(article => ({ ...article }));
}
...
function articleSlugFromPath(path) {
  ...
  return m && ARTICLE_SLUG_RE.test(m[1]) ? m[1] : null;
}

// ────────────────────── Буфер статистики ──────────────────────
//
// Бесплатный тариф KV даёт 1000 записей в сутки. Раньше каждый просмотр
// страницы делал 4–5 put (дневная статистика, слот, счётчики, просмотры
// статьи), и ~200 посетителей исчерпывали лимит. Теперь события не пишутся
// сразу, а складываются в Durable Object (StatsBuffer) и раз в 10 минут
// применяются к KV пачкой: сколько бы событий ни пришло, на пачку уходит
// по одному put на затронутый день, слот и статью. Ключи и формат данных в
// KV не изменились — админка и отчёты читают их как раньше.
//
...
async function trackStats(env, ctx, item) {
  ...
  await flushBufferedStats(env, [item]);
}
...
function newDayData(dayKey) {
  return {
    ...
    apps: {
      ...
      by: { views: 0, clicks: 0, installs: 0, sources: {}, targets: {}, campaigns: {}, platforms: {}, countries: {} },
      rs: { views: 0, clicks: 0, installs: 0, sources: {}, targets: {}, campaigns: {}, platforms: {}, countries: {} },
    },
  };
}

// Данные дня из KV в актуальном формате (старые записи без apps — дополняем).
function normalizeDayData(dayData, dayKey) {
  if (!dayData) return newDayData(dayKey);
  if (!dayData.apps) {
    dayData.apps = {
      ru: {
...
function applyEventToDay(dayData, event) {
  ...
  const country = (event.country || app).toUpperCase();
...
function liveFeedEntry(event, now) {
  const app = detectAppCode(event);
  const src = event.source || 'direct';
  const plat = event.platform || 'unknown';
  const country = (event.country || app).toUpperCase();
...
export async function flushBufferedStats(env, items) {
  ...
  const feed = [];
  const ai = { req: 0, prompt: 0, candidate: 0, cost: 0, todayReq: new Map() };

  const dayOf = (now) => cachedKv(env, days, `day:${mskDayKey(now)}`, (v) => normalizeDayData(v, mskDayKey(now)));
  const slotOf = (now) => {
    const { slotType, slotDate } = getSlotInfo(now);
    return cachedKv(env, slots, `slot:${slotDate}:${slotType}`, (v) => (v && v.installsByStore ? v : emptySlotData()));
  };
  ...
  for (const item of items) {
    ...
    if (item.kind === 'analytics' && item.event) {
      ...
      if (event.type === 'view') {
        const slug = articleSlugFromPath(event.path);
      ...
      const cTokens = Number(item.candidateTokens) || 0;
      const costUsd = ((pTokens * pricing.inputPerM) + (cTokens * pricing.outputPerM)) / 1000000;
      ai.req++;
      ai.prompt += pTokens;
      ai.candidate += cTokens;
      ai.cost += costUsd;
      const dayKey = mskDayKey(now);
      ai.todayReq.set(dayKey, (ai.todayReq.get(dayKey) || 0) + 1);
      const slotData = await slotOf(now);
      slotData.aiRequests = (slotData.aiRequests || 0) + 1;
      slotData.aiCostUsd = (slotData.aiCostUsd || 0) + costUsd;
      const dayData = await dayOf(now);
      dayData.aiRequests = (dayData.aiRequests || 0) + 1;
      dayData.aiCostUsd = (dayData.aiCostUsd || 0) + costUsd;
    }
  }

  const puts = [];
  for (const [key, dayData] of days) puts.push(env.INSTALLS.put(key, JSON.stringify(dayData)));
  // Храним данные слота 7 дней
  for (const [key, slotData] of slots) {
    puts.push(env.INSTALLS.put(key, JSON.stringify(slotData), { expirationTtl: 7 * 24 * 3600 }));
  }

  // Реальные просмотры статей блога: один счётчик на slug. KV не атомарен,
  // при одновременных сбросах единицы могут потеряться — для блога это допустимо.
  for (const [slug, n] of viewsBySlug) {
    const key = `views:${slug}`;
    const cur = parseInt(await env.INSTALLS.get(key), 10) || 0;
    puts.push(env.INSTALLS.put(key, String(cur + n)));
  }
  ...
  if (ai.req) {
    const totalReq = (parseInt(await env.INSTALLS.get('ai_stats_total_req') || '0', 10)) + ai.req;
    const totalPrompt = (parseInt(await env.INSTALLS.get('ai_stats_prompt_tokens') || '0', 10)) + ai.prompt;
    const totalCandidate = (parseInt(await env.INSTALLS.get('ai_stats_candidate_tokens') || '0', 10)) + ai.candidate;
    ...
    puts.push(
      ...
      env.INSTALLS.put('ai_stats_cost_usd', (currentCost + ai.cost).toFixed(6)),
    );
    for (const [dayKey, n] of ai.todayReq) {
      const key = 'ai_stats_today_req_' + dayKey;
      const todayReq = (parseInt(await env.INSTALLS.get(key) || '0', 10)) + n;
      puts.push(env.INSTALLS.put(key, String(todayReq), { expirationTtl: 86400 * 30 }));
    }
  }
...
function analyticsBlockForApp(dayData, appFilter) {
  if (!dayData) return null;
  if (appFilter === 'all') return dayData;
  if (dayData.apps && dayData.apps[appFilter]) return dayData.apps[appFilter];
  // До разделения аналитики все события сохранялись как российские.
  if (appFilter === 'ru' && !dayData.apps) return dayData;
  return null;
}
...
async function getStatsForPeriod(env, daysCount = 7, appFilter = 'all') {
  const emptyRes = {
    ...
    previous: { views: 0, clicks: 0, installs: 0 },
    timeline: [],
    sources: [],
    targets: [],
    campaigns: [],
...
function renderAdminPage() {
  ...
  const clientJs = enhanceAdminClientJs(ADMIN_CLIENT_JS
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/admin/blog/reorder' && request.method === 'POST') {
      if (!await verifyAdminAuth(request, env)) return jsonResponse({ error: 'unauthorized' }, 401);
      let body = {};
      try { body = await request.json(); } catch (_) {}
      const changes = Array.isArray(body.changes) ? body.changes : [];
      if (changes.length !== 2) return jsonResponse({ error: 'invalid changes' }, 400);

      const articles = await getBlogArticles(env);
      for (const change of changes) {
        if (!change || typeof change.slug !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(change.datePublished || '')) {
          return jsonResponse({ error: 'invalid article change' }, 400);
        }
        const article = articles.find(item => item.slug === change.slug);
        if (!article) return jsonResponse({ error: 'article not found' }, 404);
        article.datePublished = change.datePublished;
      }
...
Официальный комментарий: ${officialExplanation || 'Нет официального комментария'}`;
        ...
        try {
          ...
          if (geminiResp.ok) {
            ...
            if (text) {
              ...
              if (usage) {
                await trackStats(env, ctx, { kind: 'ai', model: activeModel, promptTokens: usage.promptTokenCount || 200, candidateTokens: usage.candidatesTokenCount || usage.candidatesTokensDetails?.[0]?.tokenCount || 120 });
              }
            }
          } else {
    ...
    if (url.pathname === '/api/admin/ai/test' && request.method === 'POST') {
      ...
      try {
        ...
        if (geminiResp.ok) {
          const usage = data?.usageMetadata;
          await trackStats(env, ctx, { kind: 'ai', model: activeModel, promptTokens: usage?.promptTokenCount || 100, candidateTokens: usage?.candidatesTokenCount || 50 });
        }
    ...
    if (url.pathname.startsWith('/go/')) {
      ...
      const isBelarus = storeKey === 'by-gplay' || storeKey === 'by' || storeKey === 'by-web';
      const app = isSerbia ? 'rs' : isBelarus ? 'by' : 'ru';
      const storeUrls = {
        ...
        'by-gplay': 'https://play.google.com/store/apps/details?id=by.pdd.pdd_app',
        by: 'https://play.google.com/store/apps/details?id=by.pdd.pdd_app',
        ...
        'by-web': 'https://pdd-drive.online/',
        'rs-web': 'https://rs.pdd-drive.online/'
      };
      const destination = storeUrls[storeKey] || (isSerbia
        ? 'https://rs.pdd-drive.online/'
        : isBelarus ? 'https://pdd-drive.online/' : 'https://pdd-drive.ru/links/');
      ...
      const country = request.headers.get('cf-ipcountry') || app.toUpperCase();

      const targetName = (storeKey === 'googleplay' || storeKey === 'gplay' || storeKey === 'rs-gplay' || storeKey === 'rs' || storeKey === 'by-gplay' || storeKey === 'by')
        ? 'gplay'
        : (storeKey === 'apple' || storeKey === 'appstore') ? 'appstore'
        : (storeKey === 'web' || storeKey === 'rs-web' || storeKey === 'by-web') ? 'web'
        : storeKey;
    ...
    if (url.pathname === '/api/track' && request.method === 'POST') {
      ...
      await trackStats(env, ctx, { kind: 'analytics', event: body });
      return jsonResponse({ ok: true });
    }
