...
# 1. Locate ADMIN_HTML_HEAD, ADMIN_CLIENT_JS, and function renderAdminPage
pos_head_start = code.find('const ADMIN_HTML_HEAD =')
pos_js_start = code.find('const ADMIN_CLIENT_JS =')
pos_render = code.find('function renderAdminPage()')

if pos_head_start == -1 or pos_js_start == -1 or pos_render == -1:
    print('Error: Could not find markers')
    exit(1)

html_raw_token = code[pos_head_start + len('const ADMIN_HTML_HEAD = '):pos_js_start].strip()
if html_raw_token.endswith(';'):
    html_raw_token = html_raw_token[:-1].strip()

js_raw_token = code[pos_js_start + len('const ADMIN_CLIENT_JS = '):pos_render].strip()
if js_raw_token.endswith(';'):
    js_raw_token = js_raw_token[:-1].strip()

html_str = json.loads(html_raw_token)
js_str = json.loads(js_raw_token)

print('Original HTML length:', len(html_str))
print('Original JS length:', len(js_str))

# ----------------------------------------------------
# A. REMOVE ADS FROM SIDEBAR & HTML
# ----------------------------------------------------
...
html_str = re.sub(r'<!-- \d+\. (?:ADS|Управление рекламой).*?-->\s*<div id="ads-view".*?</div>\s*</div>\s*(?=<!--|<div id="users-view"|<div id="ai-view")', '', html_str, flags=re.DOTALL)
if 'id="ads-view"' in html_str:
    # Remove from <div id="ads-view" up to next view
    p1 = html_str.find('<div id="ads-view"')
    p2 = html_str.find('<div id="users-view"', p1)
    if p2 == -1:
        p2 = html_str.find('<div id="ai-view"', p1)
    if p1 != -1 and p2 != -1:
        html_str = html_str[:p1] + html_str[p2:]
        print('Removed ads-view div from HTML!')

# 3. Update top-actions to have id="top-period-actions"
html_str = html_str.replace('<div class="top-actions">', '<div class="top-actions" id="top-period-actions">')

# 4. Inject Brand App Colors & Clean Zero-Border CSS into HTML
new_css = """
    :root {
      --bg: #F8F8FA;
      --sidebar-bg: #FFFFFF;
...
# ----------------------------------------------------
# B. OVERHAUL CLIENT JS
# ----------------------------------------------------
# Remove Ads from titles and view list
...
# Replace sidebar navigation listener
new_tab_listener = """
const VIEW_TITLES = {
...
# Replace existing VIEW_TITLES and sidebar listener
js_str = re.sub(r'const VIEW_TITLES = \{.*?\};.*?document\.querySelectorAll\(\'\.sidebar-menu \.nav-item\'\)\.forEach\(btn => \{.*?\}\);\s*;?', new_tab_listener, js_str, flags=re.DOTALL)

# Remove Ads functions from JS
...
# Add complete Users Management module (with delete, solid PRO badge, exact styling)
users_module_js = """
...
# D. BACKEND ROUTES
# ----------------------------------------------------
# 1. Clean Ads backend helpers & endpoints
code = re.sub(r'// ──+ Ads Management Helper Functions ──+.*?(?=// ──+ User Profile & Premium|\Z)', '', code, flags=re.DOTALL)
code = re.sub(r'// ──+ Ads & Promo Configuration API ──+.*?(?=// ──+ User Profile & Premium|\Z)', '', code, flags=re.DOTALL)
code = re.sub(r'if \(url\.pathname === \'/api/admin/ads\'.*?(?=if \(url\.pathname === \'/api/user/sync\')', '', code, flags=re.DOTALL)
