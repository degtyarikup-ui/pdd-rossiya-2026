...
worker_path = 'server/install-notifier/worker.js'
with open(worker_path, 'r', encoding='utf-8') as f:
    text = f.read()

# 1. Extract ADMIN_HTML_HEAD string
m_html = re.search(r'const ADMIN_HTML_HEAD = (\"(?:[^\"\\]|\\.)*\");', text)
if not m_html:
    print('Failed to match ADMIN_HTML_HEAD')
    exit(1)

html_raw = json.loads(m_html.group(1))

# 2. Extract ADMIN_CLIENT_JS string
m_js = re.search(r'const ADMIN_CLIENT_JS = (\"(?:[^\"\\]|\\.)*\");', text)
if not m_js:
    print('Failed to match ADMIN_CLIENT_JS')
    exit(1)

js_raw = json.loads(m_js.group(1))

print('Original HTML length:', len(html_raw))
print('Original JS length:', len(js_raw))
...
html_raw = re.sub(r'<style>.*?</style>', '<style>' + new_css + '  </style>', html_raw, flags=re.DOTALL)
html_raw = html_raw.replace('<div class="top-actions">', '<div class="top-actions" id="top-period-actions">')

# --- MODIFY CLIENT JS ---

# 1. Update tab switching in JS to hide top-period-actions
old_nav = "if (currentFeature === 'analytics') checkAuthAndLoad();"
new_nav = """const topPeriodActions = document.getElementById('top-period-actions');
    if (topPeriodActions) {
...
if old_nav in js_raw and "topPeriodActions" not in js_raw:
    js_raw = js_raw.replace(old_nav, new_nav, 1)

# 2. Add full Users module to JS
users_module_js = """
...
# Append Users module to JS
if "loadUsersList" in js_raw:
    # remove existing Users module if any
    js_raw = re.sub(r'// ──+ Users & Premium Management ──+.*', '', js_raw, flags=re.DOTALL)

js_raw = js_raw + users_module_js

# Replace in worker.js
new_html_json = json.dumps(html_raw)
new_js_json = json.dumps(js_raw)

text = re.sub(r'const ADMIN_HTML_HEAD = \"(?:[^\"\\]|\\.)*\";', 'const ADMIN_HTML_HEAD = ' + new_html_json + ';', text)
text = re.sub(r'const ADMIN_CLIENT_JS = \"(?:[^\"\\]|\\.)*\";', 'const ADMIN_CLIENT_JS = ' + new_js_json + ';', text)

# Add Backend DELETE user route
delete_route_code = """
...
if "url.pathname === '/api/admin/users/delete'" not in text:
    pos_grant = text.find("if (url.pathname === '/api/admin/users/grant-premium'")
    if pos_grant != -1:
        text = text[:pos_grant] + delete_route_code + '\n' + text[pos_grant:]
        print('Inserted delete user backend route!')

with open(worker_path, 'w', encoding='utf-8') as f:
    f.write(text)

print('Clean build finished successfully!')
