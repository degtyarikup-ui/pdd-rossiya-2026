...
async function main(){
 ...
 // No opaque backing behind the transparent phone frame.
 const src=await loadImage(spec.paths[i]),screen=createCanvas(638,1335);
 const scale=638/src.width;
 const screenHeight=(src.height-135)*scale;
 const sc=screen.getContext('2d');sc.save();sc.scale(scale,scale);sc.drawImage(src,0,-135);sc.restore();
 ...
 checks.push({card:i+1,size:[1080,1920],screenshotInteriorChangedChannels:differences,headlineTop:150,screenBounds:[221,513,638,screenHeight],sourceCrop:[src.width,src.height-135],scaleX:scale,scaleY:scale,noOpaqueBacking:true});
